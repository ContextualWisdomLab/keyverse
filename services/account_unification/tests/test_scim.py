...
class _TestUserOperationLocks:
    ...
    def __init__(self) -> None:
        """Create an empty keyed lock registry."""
...
class _BlockingReplaceApi(MockProductKeycloakAdminApi):
    ...
    def replace_user(self, user_id, user) -> None:
        ...
        if not self.allow_replace.wait(timeout=5):
            raise AssertionError(
                "test did not release the SCIM replacement"
            )
    ...
    def set_user_attribute(
        ...
        if (
            user_id == "dup"
            and key == TOMBSTONE_ATTRIBUTE_KEY
        ):
...
def test_scim_create_provisions_into_keycloak(
    client, api
):
...
def test_scim_create_duplicate_conflicts(client):
...
def test_scim_get_and_filter_user(client):
...
def test_scim_replace_updates_user(client, api):
    ...
    assert response.status_code == 200
    assert api.get_user(created["id"]).email == (
        "jane.doe@corp.com"
    )


def test_scim_replace_refuses_tombstone_resurrection(
    client, api
):
...
def test_scim_replace_is_serialized_with_merge(
    ...
    """Merge cannot enter between SCIM tombstone check and PUT."""
    ...
    with (
        ...
        serialized = not api.tombstone_started.wait(
            timeout=0.25
        )
...
def test_scim_delete_deprovisions_by_disabling(
    client, api
):
