...
def test_put_returns_metadata_and_plaintext_read_is_not_an_admin_route(client):
    ...
    assert "sk-live-123" not in get_response.text


def test_get_secret_route_is_unavailable_even_when_secret_is_missing(client):
...
def test_delete_secret_then_get_returns_404(client):
    ...
    assert client.get("/keyvault/ns/key1").status_code == 405
...
def test_unauthenticated_request_is_rejected(client):
    ...
    response = unauth.get("/keyvault/ns")
...
def test_keyvault_unconfigured_returns_503_not_404(unconfigured_client):
    ...
    response = unconfigured_client.get("/keyvault/ns")
