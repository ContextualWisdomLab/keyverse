...
5. Rotation accepts only an active, unexpired predecessor, validates the
   ...
   are persisted through one atomic KV-store batch; an audit failure restores
   the predecessor and removes the replacement through one atomic upsert/delete
   operation. Revoked, rotated, and expired predecessors fail closed with a
   conflict response.
