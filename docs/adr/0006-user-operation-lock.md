...
Keyverse uses one cross-process user-operation lock boundary for account merge/link, SCIM `PUT /Users/{id}` full replacement, and SCIM `PATCH /Users/{id}` with `active=false`. These operations serialize consistently, preserve tombstone/survivor invariants, and can be retried or recovered from observed durable state. A lock timeout returns retryable SCIM `503` without entering the mutation sequence.
...
- Merge, SCIM full replacement, and active=false deprovisioning share one documented concurrency authority.
- PATCH lock contention is a retryable service-unavailable outcome; it is not evidence that a partial mutation occurred.
